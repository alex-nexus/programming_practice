...
class Sudoku:
    def __init__(self, sudoku_string):
        self.board = NP.fromstring(
            sudoku_string, dtype=int, sep=' ').reshape(9, 9)
    ...
    def get_candidates_by_cell(self, i, j):
        ...
        i /= 3
        j /= 3  # floor to the multiple of 3
        # filter out number in its square
        candidates -= set(self.board[3 * i:3 *
                          (i + 1), 3 * j:3 * (j + 1)].ravel())
        return candidates
